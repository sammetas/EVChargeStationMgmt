...
public class EVCSMCompanyController {
    ...
    public ResponseEntity<Company> saveCompany(@RequestBody Company company){
        if(validate(company)) {
            evcsmService.processAndSaveCompany(company);
    ...
    public ResponseEntity<Company> updateCompany(@RequestBody Company company){
        if(validate(company)) {
       boolean isUpdated=  evcsmService.processAndUpdateCompany(company);
       if(isUpdated) {
    ...
    public ResponseEntity<Company> deleteCompany(@PathVariable int companyId){
       boolean isDeleted= evcsmService.processAndDelete(companyId);
       if(isDeleted){
    ...
    public ResponseEntity<List<Company>> getAllCompanies(){
        List<Company> list=evcsmService.getAllCompanies();
        if(!list.isEmpty()){
